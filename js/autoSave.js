...
async function requestDataStorageAccess() {
    ...
    try {
        // Prompt user to select the project root folder (where data_storage exists)
        const handle = await window.showDirectoryPicker({
            mode: 'readwrite',
            startIn: 'downloads'
        });
        
        // Check if data_storage folder exists, create if not
        const dataStorageName = 'data_storage';
        let dataStorageDir = null;
        
        try {
            dataStorageDir = await handle.getDirectoryHandle(dataStorageName, { create: false });
        } catch (e) {
            // Create if doesn't exist
            dataStorageDir = await handle.getDirectoryHandle(dataStorageName, { create: true });
        }
        
        // Create subdirectories if they don't exist
        const subdirs = ['facilitators', 'notetakers', 'white-cells', 'team_submissions'];
        for (const subdir of subdirs) {
            try {
                await dataStorageDir.getDirectoryHandle(subdir, { create: true });
            } catch (e) {
                console.error(`Error creating subdirectory ${subdir}:`, e);
            }
        }
        
        dataStorageHandle = { root: handle, dataStorage: dataStorageDir };
        localStorage.setItem('dataStorageHandle', JSON.stringify({ granted: true, timestamp: Date.now() }));
        showToast('Data storage folder access granted! Auto-save enabled.');
        setAutoSaveEnabled(true);
        return true;
    } catch (error) {
        if (error.name !== 'AbortError') {
            console.error('Error requesting directory access:', error);
            showToast('Could not access data storage folder. Using download fallback.');
        }
        return false;
    }
}
